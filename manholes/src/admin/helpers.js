...
export function buildOptionsEditorModal(adminConfig, t, title, cfgKey, specs) {
  ...
  optsWrap.innerHTML = specs.filter(s => s.optionsKey).map(spec => {
    ...
    const rows = opts.map((o) => `<tr>
      <td class="opt-enabled"><input type="checkbox" ${o.enabled!==false?'checked':''} data-opt-enabled="${cfgKey}:${spec.optionsKey}"/></td>
      <td class="opt-label"><input type="text" value="${o.label}" data-opt-label="${cfgKey}:${spec.optionsKey}"/></td>
      <td class="opt-code"><input type="text" value="${o.code}" data-opt-code="${cfgKey}:${spec.optionsKey}"/></td>
      <td class="opt-actions"><button class="btn btn-danger btn-sm" title="${t('admin.delete')}" aria-label="${t('admin.delete')}" data-opt-del="${cfgKey}:${spec.optionsKey}">×</button></td>
    </tr>`).join('');
    return `
      <div class="field">
...
export function buildOptionsEditorScreen(adminConfig, t, title, cfgKey, specs) {
  ...
  const includeDiv = document.createElement('div');
  includeDiv.className = 'admin-group';
  includeDiv.innerHTML = `<div class="admin-subtitle">${t('admin.includeTitle')}</div><div class="admin-desc">${t('admin.includeDesc')}</div>` + Object.keys(include).map(k => {
    const checked = include[k] ? 'checked' : '';
    const id = `inc_${cfgKey}_${k}`;
  ...
  const defaultsDiv = document.createElement('div');
  defaultsDiv.className = 'admin-group';
  defaultsDiv.innerHTML = `<div class="admin-subtitle">${t('admin.defaultsTitle')}</div><div class="admin-desc">${t('admin.defaultsDesc')}</div>` + specs.map(spec => {
    const current = defaults[spec.key] ?? '';
    if (spec.type === 'select') {
      const opts = adminConfig[cfgKey].options[spec.key] || [];
      const optionsHtml = [`<option value="">${t('labels.optional')}</option>`].concat(
