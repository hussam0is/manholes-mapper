...
export async function getOrCreateUser(userId, userData = {}) {
  ...
  if (result.rows[0]) {
    // SECURITY: If a user exists by email but with a different auth ID, do NOT
    // overwrite the ID. This prevents account takeover when email verification
    // is disabled. Log the mismatch for investigation and return the existing record.
    if (result.rows[0].id !== userId) {
      console.warn(`[DB] Auth ID mismatch for email ${email}: existing=${result.rows[0].id}, attempted=${userId}. Skipping ID overwrite.`);
    }
    return result.rows[0];
  }
