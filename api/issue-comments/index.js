...
export default async function handler(req, res) {
  ...
  // Route to notifications sub-handler (merged from /api/notifications)
  const action = req.query?.action || new URL(req.url, 'http://localhost').searchParams.get('action');
  if (action === 'notifications') {
    return handleNotifications(req, res, request);
  }
...
/**
 * Handle notifications: GET/POST /api/issue-comments?action=notifications
 * (Merged from api/notifications/index.js)
 */
async function handleNotifications(req, res, request) {
  try {
    await ensureDb();
    ...
    return handleApiError(err, res, '[API /api/issue-comments?action=notifications]');
  }
}
